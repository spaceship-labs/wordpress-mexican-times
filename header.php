	...
	<?php if( tie_get_option( 'top_search' ) ): ?>
					<div class="search-block">
						<form method="get" id="searchform-header" action="<?php echo home_url(); ?>/">
							<button class="search-button" type="submit" value="<?php if( !$is_IE ) _eti( 'Buscar' ) ?>"><i class="fa fa-search"></i></button>	
							<input class="search-live" type="text" id="s-header" name="s" value="<?php _eti( 'Buscar' ) ?>" onfocus="if (this.value == '<?php _eti( 'Buscar' ) ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _eti( 'Buscar' ) ?>';}"  />
						</form>
					</div><!-- .search-block /-->
	<?php endif;
		...
		<?php if( tie_get_option( 'stick_nav' ) ) $stick = ' class="fixed-enabled"' ?>
			...
			<nav id="<?php echo $navID; ?>"<?php echo $stick; ?>>
				...
				<?php endif ?>
					...
					<a href="<?php echo home_url(); ?>/?tierand=1" class="random-article ttip" title="<?php _eti( 'El Universo quiere que sepas' ) ?>"><i class="fa fa-random"></i></a>
					<?php endif ?>
